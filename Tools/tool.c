...
signed short PactUpCom_Analysis(unsigned char *bCheckBuf, unsigned char bChecklen, SPACK_UPCOM *pack_UpCom)
{
  ...
  for(bit=0; bit+12<=bChecklen; bit++)
  {
    ...
    {
      dlen = bCheckBuf[bit+9] ;               //得到数据域长度
    

      if(bit+dlen+12 <= bChecklen)              //判断是否收到指定长度字节数
      {
        flg++ ;
        if(bCheckBuf[bit+9+dlen+2] == 0x16)       //判断结束符
        {
          if(Cs_Check(DEF_CHECKSUM_CHECK, bCheckBuf, bit, dlen+10) == 1)  //CS校验正确
          {
...
//              printf("DEF_PACK_UPCOM_645_RECV_OK\n");
              ...
              return DEF_PACK_UPCOM_645_RECV_OK ;     //收到正确645帧
...
#endif
            return DEF_PACK_UPCOM_PCS_ERR ;     //CS校验错误
...
#endif
    return  DEF_PACK_UPCOM_PHEAD_ERR ;        //未收到帧头
    
  }
  return DEF_PACK_UPCOM_PEND_ERR ;                //未收到帧尾
}
